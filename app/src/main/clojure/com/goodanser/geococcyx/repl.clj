(ns com.goodanser.geococcyx.repl
            ...
            (let [result (try
                           (let [out (java.io.StringWriter.)
                                 ...
                                 val (binding [*out* out *err* err
                                               *ns* (find-ns 'com.goodanser.geococcyx.app)]
                                       (load-string code))
                                 out-str (str out)
                                 err-str (str err)]
...
(defn- slurp-file [^Uri uri]
)

(defn select-file [ctx]
  (activity/start-activity-for-result
   ctx
   Intent/ACTION_OPEN_DOCUMENT
   :on-result (fn [_act _code data]
                (when-let [uri (.getData data)]
                  (-> uri slurp-file do-eval)))))
